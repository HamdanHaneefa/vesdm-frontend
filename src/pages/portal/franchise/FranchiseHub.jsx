...
import { 
  LayoutDashboard, UserPlus, ClipboardList, Upload, Users, Book,
  LogOut, Menu, X, Bell 
} from 'lucide-react';
...
const FranchiseHub = ({ currentUser, onLogout }) => {
  ...
  const menuItems = [
    ...
    { id: 'publish-results', label: 'Publish Results', icon: Upload, path: '/portal/franchise/publish-results' },
        ...
        <div className="p-6 border-b border-slate-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Franchise Portal</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex items-center gap-3 px-4 py-3 bg-slate-800 rounded-xl mb-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm truncate">{currentUser?.name || 'Franchise'}</p>
              <p className="text-xs text-slate-400 truncate">{currentUser?.email || 'N/A'}</p>
            </div>
          </div>
          <button
        ...
        {showMobileMenu && (
          <>
            ...
            <motion.div
              ...
              animate={{ x: 0 }}
              exit={{ x: -300 }}
              className="fixed left-0 top-0 bottom-0 w-64 bg-[#0F172A] text-white z-50 lg:hidden overflow-y-auto"
            >
